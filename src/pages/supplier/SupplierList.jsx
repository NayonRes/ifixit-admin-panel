...
const SupplierList = () => {
  ...
  const clearFilter = (event) => {
    ...
    setPage(0);
    const newUrl = `/api/v1/supplier?limit=${rowsPerPage}&page=1`;
  ...
  const getData = async (pageNO, limit, newUrl) => {
    ...
    if (newUrl) {
      ...
      url = `/api/v1/supplier?name=${name}&mobile=${number}&email=${email}&startDate=${newStartingTime}&endDate=${newEndingTime}&status=${newStatus}&limit=${newLimit}&page=${
        newPageNO + 1
      }`;
    }

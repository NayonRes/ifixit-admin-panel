...
const WarrantyDetails = ({ clearFilter }) => {
  ...
  const getWarrantyData = async () => {
    ...
    let allData = await getDataWithToken(url);
    console.log("warranty data -------------------", allData?.data?.data);
  ...
  const totals = calculateTransactionTotals(
    warrantyDetails?.transaction_histories_data
  );
  useEffect(() => {
    getData();
    getWarrantyData();
    ...
    <>
        ...
        <Box sx={{ padding: "12px", margin: "16px" }}>
          <Grid container spacing={2}>
            <Grid size={6}>
              ...
              >
                Repair No : <b>{details?.repair_id}</b>
              </Typography>
            </Grid>
            <Grid size={6}>
              <Typography
              ...
              >
                Warranty No :{" "}
                <b>
                  {warrantyDetails?.warranty_id?.length > 0
                    ? warrantyDetails?.warranty_id
                    : "---------"}
                </b>
              </Typography>
            </Grid>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={4}>
              ...
              >
                Repair By :{" "}
                <b>
                  {warrantyDetails?.repair_by_data?.length > 0
                    ? warrantyDetails?.repair_by_data[0]?.name
                    : "---------"}
                </b>
              </Typography>
            </Grid>
            <Grid size={4}>
              ...
              >
                Repair Status :{" "}
                <b>
                  {warrantyDetails?.repair_status?.length > 0
                    ? warrantyDetails?.repair_status
                    : "---------"}
                </b>
              </Typography>
            </Grid>
            <Grid size={4}>
              ...
              >
                Delivery Status :{" "}
                <b>
                  {warrantyDetails?.delivery_status?.length > 0
                    ? warrantyDetails?.delivery_status
                    : "---------"}
                </b>
              </Typography>
            </Grid>
      ...
      >
        <Box sx={{ padding: "12px", margin: "16px" }}>
          <Grid container spacing={2}>
            ...
            <Grid size={12}>
              {warrantyDetails?.payment_info?.length > 0 ? (
                warrantyDetails?.payment_info?.map((item, index) => (
                  <Typography
                    variant="medium"
                    color="text.main"
                    gutterBottom
              ...
              {warrantyDetails?.payment_info?.length > 0 && (
                <>
                  {warrantyDetails?.due_amount > -1 && (
                    ...
                    >
                      Due Amount : <b>{warrantyDetails?.due_amount}</b>
                    </Typography>
                  )}

                  {warrantyDetails?.discount_amount > -1 && (
                    <Typography
                      variant="medium"
                      color="text.main"
                      gutterBottom
                    ...
                    >
                      Discount Amount :{" "}
                      <b>{warrantyDetails?.discount_amount}</b>
                    </Typography>
                  )}
                  ...
                  >
                    Total Paid Amount :{" "}
                    <b>
                      {warrantyDetails?.service_charge -
        ...
        <Box sx={{ padding: "12px", margin: "16px" }}>
          ...
          <TableContainer>
            <Table stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {!loading &&
                  warrantyDetails?.repair_attached_spareparts_data?.length >
                    0 &&
                  warrantyDetails?.repair_attached_spareparts_data?.map(
                    (row, i) => (
                      <TableRow
                        key={i}
      ...
      >
        <Box sx={{ padding: "12px", margin: "16px" }}>
          ...
          {!loading &&
            warrantyDetails?.transaction_histories_data?.length > 0 &&
            warrantyDetails?.transaction_histories_data?.map((item, i) => (
              <Box
                sx={{
                  mb: 2,
                  background: "#f9f9f9",
